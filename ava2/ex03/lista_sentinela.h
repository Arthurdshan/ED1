...
void lista_intercalar(Lista* a, Lista* b){
// IMPLEMENTAR  
    No* auxA = a->sentinela->prox;
    No* auxB = b->sentinela->prox;

    while(auxB != b->sentinela){
     
    if(auxB->dado > auxA->dado && auxA != a->sentinela) auxA = auxA->prox;
        
    else {
        desconecta_no(auxB);
        
        if(auxB->dado < auxA->dado){
            insere_esquerda(auxA, auxB);
        }
        
        else lista_inserirFim(a, auxB->dado);
        
        auxB = b->sentinela->prox;
        b->qtde--;
    }
    } 
}
